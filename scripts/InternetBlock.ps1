...
Write-Host Hosts file : "$HostFile"
Write-Host "`n"
if(!((Get-ItemProperty $HostFile).IsReadOnly)) {
	Write-Host Cannot write in host file because it is in read-only.
	Pause
    Exit
}
...
ForEach($BlockedAddress in $BlockedAddresses) {
    ...
    if($Found) {
		try {
			Add-Content -Value ((Select-String -Path $HostFile -Pattern $('^' + "$LocalAddress $BlockedAddress" + '$') -NotMatch -CaseSensitive).Line) -Path "$HostFile.tmp"
		} catch {
